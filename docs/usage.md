...
`vela chat`은 Anthropic SDK로 Claude와 직접 대화하는 **풀스크린 인터랙티브 TUI 에이전트**입니다. alternate buffer에서 실행되며, 3-패널 레이아웃(헤더/메시지/입력)과 우측 사이드바 대시보드를 제공합니다.
...
│ Session: abc-123 (15 msgs)   │
└──────────────────────────────┘
```

터미널 폭 60열 미만에서는 자동으로 숨겨집니다.

### 메시지 스크롤

스트리밍 중에도 이전 메시지를 자유롭게 탐색할 수 있습니다:

| 단축키 | 동작 |
|--------|------|
| `↑` / `↓` | 1줄 스크롤 |
| `PageUp` / `PageDown` | 뷰포트 단위 스크롤 |

새 메시지 전송 시 자동으로 하단으로 이동합니다.

### 비동기 메시지 큐

Claude가 스트리밍 응답 중이어도 **새 메시지를 입력하고 전송**할 수 있습니다:

- 최대 5개 메시지를 큐에 저장
- 현재 응답 완료 후 순차 처리
- 입력창 옆에 `queued: N/5` 표시
...
`vela chat`은 20가지 VK-*/VG-* gate를 ESM 모듈로 내장합니다. `.vela/` 프로젝트에서 실행하면 파이프라인 거버넌스가 tool 실행 전에 자동 적용됩니다.
