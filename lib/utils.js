...
function requireVela(projectDir) {
  if (!isVelaProject(projectDir)) {
    console.error('\n⛵ [Vela] Not a Vela project. Run `vela init` first.\n');
    process.exit(1);
  }
}
...
function runEngine(projectDir, cmd) {
  ...
  if (!fs.existsSync(enginePath)) {
    console.error('\n⛵ [Vela] Engine not found. Run `vela init` first.\n');
    process.exit(1);
  }
  try {
    ...
    console.error(`\n⛵ [Vela] Engine error: ${stderr || e.message}\n`);
    process.exit(1);
  }
}
...
module.exports = { velaDir, isVelaProject, requireVela, runEngine, findActivePipeline, box };
