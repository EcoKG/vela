...
module.exports = function cmdInit(projectDir, velaSrc, args) {
  ...
  // 5. Auto-commit Vela files so working tree is clean for pipeline start
  console.log(step(5, 'Committing Vela setup...'));
  try {
    execSync('git add -A', { cwd: targetDir, stdio: 'pipe' });
    execSync('git commit -m "chore: install Vela Engine v3.0"', { cwd: targetDir, stdio: 'pipe' });
    console.log(ok('Vela files committed'));
  } catch (_) {
    console.log(warn('Auto-commit skipped (not a git repo or nothing to commit)'));
  }
