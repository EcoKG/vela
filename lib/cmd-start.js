...
module.exports = function cmdStart(projectDir, args) {
  ...
  if (result.ok) {
    ...
    const stepNames = rawSteps.map(s => (typeof s === 'object' ? s.id || s.name : s));
    ...
    box([
      ...
      `${boldText('Steps:')} ${stepNames.join(' → ')}`,
      '',
      `${dimText('Launching Claude Code...')}`,
    ]);
    console.log('');

    // Spawn Claude Code with pipeline context
    const { spawn } = require('child_process');
    const claudePrompt = `Vela 파이프라인이 시작되었습니다. /vela 로 진행하세요.`;

    try {
      const claude = spawn('claude', [claudePrompt], {
        cwd: projectDir,
        stdio: 'inherit'
      });
      claude.on('error', () => {
        console.log(info(`Claude Code를 수동으로 실행하세요:`));
        console.log(`  ${dimText('$')} cd ${projectDir} && claude\n`);
      });
    } catch (_) {
      console.log(info(`Claude Code를 수동으로 실행하세요:`));
      console.log(`  ${dimText('$')} cd ${projectDir} && claude\n`);
