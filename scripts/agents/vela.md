...
→ JSON 템플릿: `.vela/references/interactive-ui.md` "프롬프트 최적화 — 보완 항목 선택" 참조
...
→ JSON 템플릿: `.vela/references/interactive-ui.md` "프롬프트 최적화 — 조립 후 확인" 참조
...
→ JSON 템플릿: `.vela/references/interactive-ui.md` "세션 재개" 참조
...
→ JSON 템플릿: `.vela/references/interactive-ui.md` "Research 방식 선택" 참조
...
→ JSON 템플릿: `.vela/references/interactive-ui.md` "파이프라인 규모 선택" 참조
...
모든 UI JSON 템플릿은 `.vela/references/interactive-ui.md`에 정의되어 있다.
각 단계에 진입할 때 해당 섹션을 읽고 AskUserQuestion을 호출한다.

### Checkpoint (사용자 승인) 단계
plan.md 요약을 보여주고 승인을 묻는다.
→ `.vela/references/interactive-ui.md` "Checkpoint" 참조
- "승인" → `record pass` + `transition`
- "변경 요청" → 사용자 피드백 받고 plan 단계로 돌아가 Planner 재소환
- "파이프라인 취소" → `cancel`

### Commit (커밋 메시지 확인) 단계
→ `.vela/references/interactive-ui.md` "Commit 메시지 확인" 참조

### Finalize (PR 생성) 단계
→ `.vela/references/interactive-ui.md` "Finalize" 참조

### Cancel (취소 확인)
→ `.vela/references/interactive-ui.md` "Cancel 확인" 참조

### PM 거부 시 사용자 알림
PM이 reject을 결정하면, 자동 재시도하지 말고 사용자에게 보여준다.
→ `.vela/references/interactive-ui.md` "PM 거부 시" 참조
...
| 조건 | 실행 방식 | model 파라미터 | 이유 |
|------|----------|---------------|------|
| 경쟁가설 디버깅 (리서치) | **Teammate** | `"opus"` | 가설 반박/검증에 SendMessage 필수 |
| 다중 파일/모듈 동시 수정 | **Teammate** | `"sonnet"` | 파일 충돌 방지를 위한 실시간 소통 |
| CrossLayer 개발 (프론트+백+DB) | **Teammate** | `"sonnet"` | 계층 간 인터페이스 조율 |
