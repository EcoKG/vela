...
다음 중 하나라도 해당하면 최적화가 필요하다:
- 대상이 불명확 ("코드 수정해줘" — 어떤 코드?)
- 범위가 불명확 ("리팩토링해줘" — 어디까지?)
- 목적이 불명확 ("개선해줘" — 무엇을 개선?)
- 기술적 맥락 부족 ("버그 수정" — 어떤 버그? 재현 조건?)

### 최적화 흐름

1. **프롬프트 분석** — 대상, 범위, 목적, 기술적 맥락이 충분한지 판단
2. **부족하면 AskUserQuestion으로 보완 요청**:

```json
{
  "questions": [{
    "question": "작업을 더 구체적으로 지정하면 더 정확하게 진행할 수 있습니다. 어떤 부분을 보완할까요?",
    "header": "⛵ Prompt",
    "options": [
      {
        "label": "대상 파일/모듈 지정",
      ...
      {
        "label": "이대로 진행 (충분함)",
        "description": "현재 프롬프트로 충분합니다. 바로 시작합니다."
      }
    ],
    "multiSelect": false
  }]
...
3. **사용자가 보완하면** → 보완된 프롬프트로 진행
4. **"이대로 진행"** → 원본 프롬프트로 진행

### 프롬프트가 충분한 경우

다음 조건을 모두 충족하면 최적화 없이 바로 진행:
- 구체적 대상이 명시됨 (파일명, 클래스명, 함수명 등)
- 작업 목적이 명확 (수정/추가/삭제/분석 등)
- 기술적 맥락이 충분 (에러 메시지, 요구사항 등)

예시 (최적화 불필요):
- "src/auth/jwt.js의 토큰 만료 시간을 15분에서 30분으로 변경"
- "WSAPollServer.cpp의 ProcessItem에서 _stricmp를 _strnicmp로 교체하고 길이 비교 추가"
- "회원가입 API에 이메일 중복 검증 추가"
